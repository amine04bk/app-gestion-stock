...
function DetailCommande() {
  ...
  const [ligneCommandes, setLigneCommandes] = useState([
    { ressourceId: "", qte: 0, mnt: 0, pu: 0, projetId: "" },
  ]);
...
/*   const [projects, setProjects] = useState([]); */
  ...
  // Fetch fournisseurs, products, and projects on component load
  useEffect(() => {
      ...
      const fetchCommande = async () => {
        ...
        setLigneCommandes(
          ligneCommandes.map((lf) => ({
            ...
            /* projetId: lf.projectId ? lf.projectId : "", */
            qte: lf.qte,
            pu: lf.pu,
            mnt: lf.pu * lf.qte,
          }))
                    ...
                    <Card.Body>
                      ...
                      <div id="capture">
                        ...
                        <Table striped bordered hover>
                          ...
                          <tbody>
                            {ligneCommandes.map((ligneCommande, index) => (
                              <tr key={index}>
                                <td>
                                  {
                                    ressources.find(
                                      (p) => p.id === ligneCommande.ressourceId
                                    )?.name
                                  }
                                </td>
                               {/*  <td>
                                ...
                                <td>{ligneCommande.pu}</td>
                                <td>{ligneCommande.qte}</td>
                                <td>{ligneCommande.mnt}</td>
                              </tr>
                            ))}
                          </tbody>
                        </Table>

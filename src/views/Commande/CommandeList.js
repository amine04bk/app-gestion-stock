...
import { commandeDeleted, getCommandes } from "../../Redux/commandeReduce";
...
function ListCommande() {
    ...
    },
    {
      ...
      Cell: ({ cell, row }) => (
        <div className="actions-right block_action">
          <Button
            onClick={() => {
              navigate.push("/commande/update/" + cell.row.original.id);
            }}
            variant="warning"
            size="sm"
          ...
          <Button
            onClick={() => {
              navigate.push("/commande/detail/" + cell.row.original.id);
            }}
            variant="info"
            size="sm"
  ...
  function ajouter() {
    navigate.push("/commande/add");
  }

  const fetchCommandes = useCallback(async () => {
    var response = await dispatch(getCommandes({ typeCommande: 1 }));
    var data = await response.payload.data;
    setEntities(data);
  }, [dispatch]);

  function commandeDelete(id) {
    dispatch(commandeDeleted(id))
      .unwrap()
      .then((response) => {
  ...
  const confirmMessage = (id, e) => {
    setAlert(
      <SweetAlert
        ...
        title="Vous éte sure de supprime ce commande?"
        onConfirm={() => commandeDelete(id, e)}
        onCancel={() => hideAlert()}
